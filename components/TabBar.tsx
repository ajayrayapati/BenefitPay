...
export const TabBar: React.FC<TabBarProps> = ({ currentView, onChange }) => {
      ...
      case AppView.WALLET:
        return (
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className={`w-6 h-6 ${color}`}>
            <path d="M11.25 4.533A9.707 9.707 0 006 3.755c-2.348 0-4.526.918-6.191 2.421l8.691 8.691 8.692-8.691A9.688 9.688 0 0011.25 4.533z" fillOpacity={0} /> {/* Spacer logic placeholder for real icon shapes */}
  ...
  return (
    <div className="absolute bottom-0 w-full bg-white/90 backdrop-blur-md border-t border-gray-200 pb-safe pt-2 px-6 flex justify-between items-center z-50 h-[83px]">
      <button 
        onClick={() => onChange(AppView.WALLET)}
        className="flex flex-col items-center justify-center w-1/3 space-y-1 active:scale-95 transition-transform"
      >
        {getIcon(AppView.WALLET, currentView === AppView.WALLET)}
        <span className={`text-[10px] font-medium ${currentView === AppView.WALLET ? 'text-blue-500' : 'text-gray-400'}`}>Wallet</span>
      </button>
      ...
      >
        ...
        <span className={`text-[10px] font-medium ${currentView === AppView.RECOMMEND ? 'text-blue-500' : 'text-gray-400'}`}>Ask AI</span>
      </button>
      ...
      >
        ...
        <span className={`text-[10px] font-medium ${currentView === AppView.ADD_CARD ? 'text-blue-500' : 'text-gray-400'}`}>Add Card</span>
      </button>
    </div>
  );
};
